...
<div class="container">
    <div class="subheader">
        <h1 class="subheader-title">
            <i class='subheader-icon fal fa-table'></i>Franchises: <span class='fw-300'>Plots</span>
        </h1>
    </div>
    <div class="row">
        <div class="col-md-12">
        ...
        @foreach ($franchises_plots as $plot)   
            <div class="col-md-12">
                <form method="post" action="{{ route('plots.franchises.accept',$plot->id) }}">
                    @csrf
                    <div class="form-row">
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="plottype">Created By</label>
                                <p>{{ $plot->user->name ?? 'Not Given' }}</p>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="plottype">Franchise Name</label>
                                <p>{{ $plot->user->franchise->name ?? 'Not Given' }}</p>
                            </div>
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="col-md-4 ml-auto d-flex justify-content-end">
                            <a href="{{ route('plots.franchises.reject',$plot->id) }}" class="btn btn-sm btn-danger mt-4 mr-3">Reject</a>
                            <button type="submit" id="savePlotbtn"  class="btn btn-success mt-4">Accept</button>
                        </div>
                    </div>
                </form>
            </div>
